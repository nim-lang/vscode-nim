...
proc startLanguageServer(tryInstall: bool, state: ExtensionState) {.async.} =
  ...
  if rawPath.isNil or not fs.existsSync(path.resolve(rawPath)):
    ...
    if tryInstall and not state.installPerformed:
      ...
      let cantInstallInfoMesssage: cstring = "Unable to find/install `nimlangserver`. You can attempt to isnstall it by running `nimble install nimlangserver` or downloading the binaries from https://github.com/nim-lang/langserver/releases."
